...
(async () => {
  ...
  const { createServer } = await import(pathToFileURL(path.join(projectRoot, 'node_modules', 'vite', 'dist', 'node', 'index.js')).href);
  const config = await loadViteConfig({ mode: 'development', command: 'serve' });
  const port = Number(readArgValue('--port', '8080')) || 8080;
